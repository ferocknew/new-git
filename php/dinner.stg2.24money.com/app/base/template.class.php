...
class base_template {
	...
	private static $img_cdn_count = 1;
		...
		elseif ($tag{0} == '/') {
			...
			switch ($tag_sel) {
				...
				case "array_rand" :
					...
					return '<?php echo $this->html_array_rand(' . $t[from] . '); ?>';
					break;
		...
		if (!empty($moddb)) {
			foreach ($moddb AS $key => $mod) {
				...
				switch ($s[0]) {
					...
					case 'truncate' :
						$p = 'framework_static_base::sub_str(' . $p . ",$s[1])";
						break;
	...
	function smarty_prefilter_preCompile($source) {
		...
		if ($file_type == '.dwt') {
			...
			/* 检查有无动态库文件，如果有为其赋值 */
			// $dyna_libs = base_main::get_dyna_libs($GLOBALS['_CFG']['template'], $this -> _current_file);
			if ($dyna_libs) {
	 ...
	 * @author	jonah.fu
	 */
	public function html_array_rand($arr) {
	...
	function img_url($str) {
		self::$img_cdn_count++;
		if (self::$img_cdn_count > IMG_CDN_NUMS) {
			self::$img_cdn_count = 1;
		}
		return constant(CDN . self::$img_cdn_count) . '/' . $str;
	}
